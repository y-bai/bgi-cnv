...
"""
    File Name: online_call_mp_gpu_ae.py
    Description: online cnv call with multi-gpu, input dimension reduced by AE model
...
class CNVNetwork:
    ...
    def __init__(self, weight_path, win_size, n_feat, n_class, drop, _blocks, fc_size,
                 l2ratio, temp, filters, kernel_size):
        ...
        model_name = 'cnvnet'
        self.model = cnv_net(win_size, n_feat, n_class, filters=filters, kernel_size=kernel_size, strides=1, pool_size=2,
...
def pool_job(weight_path, win_size, n_feat, n_class, drop, _blocks, fc_size, in_data, batch_size,
             l2rate, temperature_scale, filters, kernel_size):
    model = CNVNetwork(weight_path, win_size, n_feat, n_class, drop, _blocks,
                       fc_size, l2rate, temperature_scale, filters, kernel_size)
    return model.predict(in_data, batch_size)


def online_call_mp_gpu_ae(online_seg_data_root_dir, online_call_out_root_dir, model_in_root_dir,
                n_win_size=625, orig_win=10000, n_feat=13, n_class=3,
                epochs=64, batch=1024, learn_rate=0.001, drop=0.5,
                fc_size=64, blocks='4_4_3', step_size=200,
                l2r=1e-4, temperature=5, lbl_smt_frac=0.1, pw=0, filters=32, kernel_size=16,
    ...
    part_fname = 'win{0}_step{1}_r{2:.2f}_chr{3}_seg_'.format(orig_win, step_size, min_ratio, chr_id)
    # create out dir
    ...
    call_out_fn = os.path.join(online_call_out_subroot_dir,
                               'win{0}_step{1}_r{2:.2f}_chr{3}-cnv-call-'.format(
                                   orig_win, step_size, min_ratio, chr_id))
    ...
    pred_h5_fn_list = glob.glob(os.path.join(online_seg_data_subroot_dir, part_fname+'pred_*_AE_dimreduce.h5'))
    logger.info("=====>predictable files: {}".format(pred_h5_fn_list))
    ...
    for i_fn in pred_h5_fn_list:
        func_parmas.append((model_weight_fn, n_win_size, n_feat,
                             n_class, drop, _blocks, fc_size, i_fn, batch, l2r, temperature, filters, kernel_size))
