...
"""
    File Name: online_cnv_merge_main.py
    Description:
...
def get_prob_map(row_start_ind, row_end_ind):  # moving average
    ...
    my_bkps = rpt.Pelt(model='rbf', min_size=min_seg_len, jump=5).fit_predict(
...
def main(args):
    ...
    un_pred_df.loc[(cnv_df['indicator'] == 1) | (cnv_df['indicator'] == 2), 'seg_e'] =\
        un_pred_df.loc[(cnv_df['indicator'] == 1) | (cnv_df['indicator'] == 2), 'seg_s'] + step_size
    ...
    out_cnv_fn = os.path.join(out_dir, 'M{}_{}_{}_{}_out_cnv_{}.csv'.format(
