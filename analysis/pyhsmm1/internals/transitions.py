...
class _HMMTransitionsBase(object):
    ...
    def alphav(self,weights):
        if weights.ndim < 2:
            ...
            for idx, distn in enumerate(self._row_distns):
                distn.alphav_0 = weights[idx,:]
...
class DATruncStickyHDPHMMTransitions(_DATruncHDPHMMTransitionsSVI):
    def __init__(self,kappa,**kwargs):
        self.kappa = kappa
        N = kwargs['num_states']
        original_alphav = np.ones(N + 1) * kwargs['alpha']
        temp_alpha = np.zeros((N + 1, N + 1))
        for row_index, delta_ij in zip(range(N + 1),np.eye(N + 1)):
            temp_alpha[row_index,:] = original_alphav + self.kappa * delta_ij
        kwargs['alpha'] = temp_alpha[:-1,:]
        super(DATruncStickyHDPHMMTransitions,self).__init__(**kwargs)
    # def _set_alphav(self,weights):
    #     for distn, delta_ij in zip(self._row_distns,np.eye(self.N + 1)):
    #         distn.alphav_0 = weights + self.kappa * delta_ij

    # alphav = property(_DATruncHDPHMMTransitionsSVI.alphav.fget,_set_alphav)
