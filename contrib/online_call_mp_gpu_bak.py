...
def pool_job(weight_path, win_size, n_feat, n_class, drop, _blocks, fc_size, in_data, batch_size):
    model = CNVNetwork(weight_path, win_size, n_feat, n_class, drop, _blocks, fc_size)
...
def online_call_mp_gpu(online_seg_data_root_dir, online_call_out_root_dir, model_in_root_dir,
    ...
    call_out_fn = call_out_fn + 'result-mp-gpu.csv'
    ...
    with mp.Pool(n_proc) as p:
        ...
        for idex, (i_pred_meta, i_ypred, i_fn) in enumerate(results):
            logger.info('finished /{}, {}'.format(idex, total_fn, i_fn))
